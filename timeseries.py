...
def analyze_review_csv(csv_path):
    ...
    # 4. 연-월(year_month) 파생변수 생성
    df['year_month'] = df['date'].dt.to_period('M').astype(str)

    # 5. 전체 연월 리스트(빈 월 포함) 생성
...
analyze_review_csv("database/reviews_aladin.csv")
# analyze_review_csv("database/reviews_kyobo.csv")
# analyze_review_csv("database/reviews_yes24.csv")
